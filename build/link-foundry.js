...
const windowsInstructions =
  process.platform === "win32" ? ' Start with a drive letter ("C:\\").' : "";
const dataPath = (
  await prompts({
    type: "text",
    name: "value",
...
const symlinkPath = path.resolve(dataPath, "modules", "pf2e-thaum-vuln");
...
if (symlinkStats) {
  ...
  const proceed = (
    await prompts({
      ...
      name: "value",
      initial: false,
      message: `A "PF2e Exploit Vulnerability" ${atPath} already exists in the "systems" subfolder. Replace with new symlink?`,
    })
  ).value;
  if (!proceed) {
